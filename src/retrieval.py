...
def build_tfidf_index(chunk_texts):
    """
    Builds a TF-IDF vectorizer and TF-IDF matrix from document chunks.
    """

    processed_chunks = [normalize_text(text) for text in chunk_texts]
...
def build_faiss_index(chunk_texts, embedding_model):
    """
    Builds a FAISS index from sentence embeddings.
    """
...
def retrieve_tfidf(query, chunks_df, vectorizer, tfidf_matrix, top_k=3):
    """
    Retrieves top-k chunks using TF-IDF similarity.
    """
...
def retrieve_hybrid(
    ...
    """
    Combines TF-IDF retrieval and embedding-based retrieval.

    TF-IDF is useful for exact keyword and acronym questions.
    Embeddings are useful for semantic and conceptual questions.
    """
